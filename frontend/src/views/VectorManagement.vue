<template>
  <div class="vector-management">
    ...
    <el-card class="collections-card">
      ...
      <el-table :data="collections" style="width: 100%" v-loading="loading" empty-text="暂无向量集合">
        ...
        <el-table-column label="操作" width="300" fixed="right">
          <template #default="{ row }">
            <el-button-group>
              <el-button size="small" @click="viewCollection(row)" :icon="View">
                查看
              </el-button>
              <el-button size="small" type="primary" @click="testSearch(row)" :icon="Search">
                测试检索
              </el-button>
              <el-button size="small" type="danger" @click="confirmDelete(row)" :icon="Delete">
                删除
              </el-button>
            </el-button-group>
          </template>
    ...
    <el-dialog v-model="showCreateDialog" title="索引新文档" width="800px" @close="resetIndexForm">
        ...
        <el-tab-pane label="📂 文件管理系统" name="select">
          <el-form :model="indexForm" label-width="120px">
            ...
            <el-form-item label="文档类型">
              ...
              <el-text size="small" type="info">选择类型后会自动设置集合名称</el-text>
            </el-form-item>
        ...
        <el-tab-pane label="✏️ 直接输入" name="text">
          <el-form :model="indexForm" label-width="120px">
            ...
            <el-form-item label="文档内容">
              <el-input v-model="indexForm.text" type="textarea" :rows="12" placeholder="输入要索引的文档内容..." />
            </el-form-item>
      ...
      <template #footer>
        ...
        <el-button type="primary" @click="indexDocument" :loading="indexing" :icon="Upload">
          开始索引
        </el-button>
      </template>
    </el-dialog>
    ...
    <!-- 检索测试对话框 -->
    <el-dialog v-model="showSearchDialog" title="测试向量检索" width="800px">
      <el-form>
        <el-form-item label="查询内容">
          <el-input v-model="searchQuery" placeholder="输入查询问题..." @keyup.enter="performSearch">
            <template #append>
              <el-button :icon="Search" @click="performSearch" :loading="searching" />
            </template>
          </el-input>
        </el-form-item>
      ...
      <div v-if="searchResults.length > 0" class="search-results">
        <el-card v-for="(result, index) in searchResults" :key="index" class="result-card" shadow="hover">
          <template #header>
...
const handleDocumentTypeChange = () => {
  ...
  ElMessage.info('已自动设置集合名称为: ' + indexForm.value.collection_name)
}
