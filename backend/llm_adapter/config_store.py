...
实现 LLM Adapter 配置的持久化存储，参考节点系统的配置管理模式
"""
...
class LLMAdapterConfigStore:
    """
    LLM Adapter 配置存储管理类

    管理 LLM Provider 配置文件的存储、加载、删除等操作
    配置文件存储在 backend/llm_adapter/configs/instances/ 目录下
    """
    ...
    def __init__(self):
        ...
        logger.info(f"LLM Adapter 配置存储目录: {self.configs_dir}")
