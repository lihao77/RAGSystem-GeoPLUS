...
from capabilities.mcp_tools import get_mcp_tools_capability
from execution.observability import ensure_request_id
from services.mcp_service import MCPServiceError
...
def list_server_templates():
    ...
    return _ok(get_mcp_tools_capability().list_templates())
...
def install_server_from_template():
    ...
    try:
        data = get_mcp_tools_capability().install_server_from_template(request.get_json(silent=True) or {})
...
def search_registry():
    ...
    try:
        result = get_mcp_tools_capability().search_registry(
            search=search,
            cursor=cursor,
            limit=limit,
...
def install_server_from_registry():
    ...
    try:
        data = get_mcp_tools_capability().install_server_from_registry(request.get_json(silent=True) or {})
...
def list_servers():
    ...
    return _ok(get_mcp_tools_capability().list_servers())
...
def add_server():
    """添加 MCP Server 配置"""
    try:
        data = get_mcp_tools_capability().add_server(
...
def update_server(server_name: str):
    ...
    try:
        status = get_mcp_tools_capability().update_server(
            server_name,
            request.get_json(silent=True) or {},
            request_id=ensure_request_id(request.headers.get('X-Request-ID')),
        )
...
def delete_server(server_name: str):
    ...
    try:
        get_mcp_tools_capability().delete_server(server_name, request_id=ensure_request_id(request.headers.get('X-Request-ID')))
    except MCPServiceError as error:
        return _err(error.message, error.status_code)
...
def connect_server(server_name: str):
    """连接到指定 MCP Server"""
    try:
        status = get_mcp_tools_capability().connect_server(server_name, request_id=ensure_request_id(request.headers.get('X-Request-ID')))
...
def disconnect_server(server_name: str):
    """断开指定 MCP Server"""
    get_mcp_tools_capability().disconnect_server(server_name, request_id=ensure_request_id(request.headers.get('X-Request-ID')))
...
def test_server(server_name: str):
    """测试连接（断开并重连）"""
    try:
        result = get_mcp_tools_capability().test_server(server_name, request_id=ensure_request_id(request.headers.get('X-Request-ID')))
...
def list_server_tools(server_name: str):
    ...
    return _ok(get_mcp_tools_capability().list_server_tools(server_name))


@mcp_bp.route('/tools', methods=['GET'])
def list_all_tools():
    """列出所有已连接 MCP Server 的工具"""
    return _ok(get_mcp_tools_capability().list_all_tools())
