...
├── implementations/react/agent.py               # ReActAgent 问答智能体
...
# 注册智能体
orchestrator.register_agent(ReActAgent())
...
# 需要 ReActAgent 查询数据 + ChartAgent 生成图表

# 1. 编排器自动选择 ReActAgent
qa_response = await qa_agent.execute(
    task="查询2023年各市的受灾人数",
    context=context
)

# 2. ReActAgent 完成后，数据存入 context
