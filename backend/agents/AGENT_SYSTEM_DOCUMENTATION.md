...
- **分层架构**:
  ...
  - **Worker Agents (执行层)**: 负责具体领域任务（如 QAAgent, GenericAgent）。
...
- **工作流程**:
  ...
  3. 实例化对应的智能体类（QAAgent, MasterAgent, GenericAgent）。
...
agents:
  ...
  generic_translator:
    agent_name: generic_translator
    display_name: 通用翻译助手
    enabled: true
    custom_params:
      type: generic
      behavior:
        role_definition: "你是一个精通多国语言的翻译专家。"
        task_prompt: "请将用户输入的文本翻译成目标语言，并保持原有语气。"
```

### 3.2 如何添加新智能体

#### 方法一：添加通用智能体 (GenericAgent)
无需写代码，只需在 YAML 中添加配置：
1. 打开 `backend/agents/configs/agent_configs.yaml`。
2. 添加新的 entry，设置 `custom_params.type: generic`。
3. 在 `custom_params.behavior` 中定义角色提示词。

#### 方法二：开发自定义智能体类
1. 在 `backend/agents/` 下创建新文件（如 `my_agent.py`）。
2. 继承 `BaseAgent` 并实现 `execute` 方法。
3. 在 `backend/agents/agent_loader.py` 的 `AGENT_TYPES` 中注册新类型。
4. 在配置文件中设置 `custom_params.type: my_agent_type`。
