...
└─────────────────────────────────────────────────────────────┘
        ...
        ↓             ↓               ↓             ↓
   ┌────────┐   ┌────────┐      ┌────────┐    ┌────────┐
   │QAAgent │   │ChartAgt│  ... │Analysis│    │Report  │
   │        │   │        │      │Agent   │    │Agent   │
   └────────┘   └────────┘      └────────┘    └────────┘
        ↓             ↓               ↓             ↓
   使用工具      生成图表          分析数据        生成报告
...
4. _analyze_task() → complexity='simple'
   ↓
5. _delegate_to_single_agent() → 委托给 QAAgent
   ↓
6. QAAgent.execute() → 查询知识图谱
   ↓
7. 返回结果给用户
```
...
5. _coordinate_multiple_agents()
   ↓
6. 分解为子任务：
   子任务1: "查询2023年各市受灾人数" → QAAgent
   子任务2: "分析最严重的三个地区" → QAAgent (依赖子任务1)
